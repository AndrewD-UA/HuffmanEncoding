...
public class Decode {
	...
	/**
	 * Given the root of a Huffman tree and the encoding, decode it to English
	 * characters.
	 * 
	...
	public static String decode(String encoding, HuffmanNode root) {
		...
		while (index < encoding.length()) {
			...
			if (currentChar == '0') {
				...
				// If the left child is a leaf, add the character associated with that node to
				// the result
				if (currNode.isLeaf()) {
			...
			// Else, travel to the right child (it must be a 1)
				...
				// If the right child is a leaf, add the character associated with that node to
				// the result
				if (currNode.isLeaf()) {
					result += currNode.getData();
					currNode = root;
	 ...
	 * 
	 * The first int is always the length of the transmissions. The remaining ints
	 * are the encoding as 1s and 0s. The ending of the transmission is assumed to
	 * be zero-padded.
	 * 
